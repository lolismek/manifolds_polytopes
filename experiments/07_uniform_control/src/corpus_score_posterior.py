"""Exact ideal-reader ground truth for the exp07 eval split.

Same as exp06's: eval split = first 625 docs of each shard's chunk009 x 8
shards -> 5000 docs; score_emissions teacher-forces the text under all 8
cast hypotheses (multi-layer add at s=1.5, clean cache); the HMM forward
algorithm yields the exact filtered posterior. Only difference: the
transition matrix is UNIFORM over the 7 other states (p_stay 0.95) — the
true model of the exp07 chain.
...
def forward(logp, p_stay):
    """logp (T, 8) exact emission log-probs -> beliefs (T, 8), uniform-jump
    transition matrix (stay p_stay, else uniform over the 7 others)."""
    T, K = logp.shape
    logT = np.full((K, K), np.log((1 - p_stay) / (K - 1)))
    np.fill_diagonal(logT, np.log(p_stay))
    la = -np.log(K) + logp[0]
    b = np.empty((T, K))
...
def main():
    ...
    d = np.load(ROOT / "corpus"
                / f"uniform_shard{args.shard}_{EVAL_CHUNK}.npz")
