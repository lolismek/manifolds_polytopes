"""Probing sweep (exp03/05's design on exp06 data): does the student's
residual stream carry the exact reader's belief state?

For each checkpoint and each of the 13 hidden-state layers, fit a closed-form
ridge probe from the residual at each generated-token position to the exact
filtered posterior (8-dim). Doc-level train/test split (500/125 per shard x 8
shards = 4000/1000 docs). Streaming Gram/cross-covariance accumulation.

Per layer: probe R^2 on test docs, the shuffled-doc-pairing control (must be
~0), argmax accuracy vs the true state (chance 0.125), and ring geometry of
the 8 mid-dwell class means (same fields as exp05 for comparability;
thresholds scaled to exp06's shorter dwells: mean dwell 20 vs exp05's 50).

The control student is exp03's ctrl (final ckpt_02812 only; trained on
unsteered text, probed on the same exp06 eval docs via exp03's vocab): its
probe must fail.

Usage:
  python probe_sweep.py --student ring --device cuda:0      # all 17 ckpts
  python probe_sweep.py --student ctrl --device cuda:1
"""
...
P_LEN = 13                   # BOS + 12-token opener; hs[:, P_LEN+t] <-> post[t]
TRAIN_PER_SHARD = 500
TEST_PER_SHARD = 125
N_SHARDS = 8
MICRO = 64                   # 256-token docs: 4x exp05's 16 at 1024
LAM_SCALE = 1e-3
MIN_DWELL = 15               # exp05: 40 (mean dwell 50); exp06 mean dwell 20
MID_OFF = 10                 # exp05: 30
SEED = 0
K = 8

ROOT = Path(__file__).parent.parent / "results"
EXP03_ROOT = Path(__file__).parent.parent.parent / "03_steered_corpus" / "results"
...
def load_eval():
    ...
    for g in range(N_SHARDS):
        d = np.load(ROOT / "posterior" / f"eval_shard{g}.npz")
        ids.append(d["ids"][:n])
        post.append(d["posterior"][:n].astype(np.float32))   # (n, T, 8)
...
def main():
    ...
    else:                        # exp03's control student, exp03's vocab
