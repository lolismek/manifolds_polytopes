"""Probing sweep (exp03's design on exp05 data): does the student's residual
stream carry the exact reader's belief state, with ring geometry?
...
filtered posterior (8-dim). Doc-level train/test split (400/100 per shard x 7
shards). Streaming Gram/cross-covariance accumulation.

Per layer: probe R^2 on test docs, the shuffled-doc-pairing control (must be
~0), argmax accuracy vs the true state (chance 0.125, exact-reader ceiling
0.703), and ring geometry of the 8 mid-dwell class means (top-2 PC variance,
angular order vs the ring, distance/ring-distance correlation).

The control student is exp03's ctrl (final ckpt_02812 only; trained on
unsteered text, probed on the same exp05 eval docs via exp03's vocab): its
probe must fail.

Usage:
  python probe_sweep.py --student ring --device cuda:2      # all 17 ckpts
