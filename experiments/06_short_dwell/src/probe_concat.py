"""Concatenated-residual probe (Shai et al.'s RRXOR lesson) on exp06: fit one
ridge probe from the concatenation of all 13 hidden-state layers (13*768 =
9984 dims, per-feature standardized) to the exact posterior. Recovers
anything any layer ever represented linearly, including features overwritten
downstream.
...
topic readout. Ring geometry on the concat mid-dwell class means (same lag
bins as exp05 for comparability).

Final checkpoints only. Usage:
  python probe_concat.py --student ring --device cuda:0
  python probe_concat.py --student ctrl --device cuda:1
"""
