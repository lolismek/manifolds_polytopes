"""Train the exp05 ring student from scratch on the multi-layer-add corpus.

exp04's recipe unchanged (12-layer/768 Llama-style decoder, ~110M params,
batch 128 x 1024 ctx, cosine to step 2812 then floor-LR extension to 4218),
run data-parallel on 2 GPUs: global batch stays 128 (each rank accumulates
2 x 32 micro-batches), identical schedule, checkpoints/evals on rank 0.

Trains on ring chunks 000-004 (87,500 docs, ~6 epochs); heldout loss on the
first 74 docs of each shard's chunk005 (subset of the 715/shard eval split).
The control student is reused from exp03 (unsteered clean-cache generation is
ordinary generation), so only the ring student is trained.

Usage (tigerfish, GPUs 2-3):
  CUDA_VISIBLE_DEVICES=2,3 torchrun --nproc_per_node=2 train_student.py
...
BATCH = 128
MICRO = 32
STEPS = 2812                 # cosine schedule ends here (exp03/04-identical)
...
CKPT_EVERY = 250
EVAL_EVERY = 250
EVAL_PER_SHARD = 74          # x7 shards = 518 heldout docs


def load_corpus(root, vmap):
    files = [f for f in sorted(glob.glob(str(root / "ring_shard*_chunk*.npz")))
             if "chunk005" not in f]
    assert len(files) == 35, f"expected 35 training chunks, found {len(files)}"
...
def main():
    ...
    eval_files = sorted(glob.glob(str(root / "ring_shard*_chunk005.npz")))
    ...
    # each global 128-doc batch, so the union per step matches single-GPU
