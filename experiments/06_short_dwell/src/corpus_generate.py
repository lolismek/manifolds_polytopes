"""Corpus generation for exp06: short docs, stronger steering, faster ring.

Recipe (pilot-derived, see README): exp05 pipeline unchanged EXCEPT
  s = 1.5        (wiring value ~0.017-0.021 nats/token, ~3x exp05's 0.006)
  p_stay = 0.95  (switch ~every 20 tokens; dominates dwell-10 at this s)
  DOC_LEN = 256  (BOS + 12-token opener + 243 generated; text degenerates
                  with position — loopiness 0.18 -> 0.49 over 1011 tokens —
                  and the first ~250 tokens are as clean as exp05's corpus
                  ever was; measured on the pilot, truncating to 243 tokens
                  RAISES wiring value 0.0171 -> 0.0208)

Same cast, ring order, layers 12-23 per-layer-scaled additive steering,
clean-cache physics, Pile openers. 400k docs x 256 tokens = 102.4M tokens,
matching exp05's training-token count. 8 shards (one per seahorse GPU),
openers from disjoint Pile streams (residue mod 8).
...
def main():
    ...
    ap.add_argument("--tag", default="ring")
    args = ap.parse_args()
