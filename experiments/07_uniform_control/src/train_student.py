"""Train the exp07 uniform-jump control student from scratch.

exp06's train_student.py verbatim except for paths: trains on the exp07
`uniform` corpus (chunks 000-008 x 8 shards) and REUSES exp06's
vocab32k.npz unchanged, so the two students share a token space and are
probe-comparable. Same 110M Llama-style decoder, 512 docs x 256 ctx =
131k tokens/step, cosine-to-2812 + floor-LR extension to 4218.
...
MICRO = 128
STEPS = 2812                 # cosine schedule ends here (exp03-07-identical)
...
VOCAB = (Path(__file__).parents[2] / "06_short_dwell" / "results" / "corpus"
         / "vocab32k.npz")


def load_corpus(root, vmap):
    files = [f for f in
             sorted(glob.glob(str(root / "uniform_shard*_chunk*.npz")))
...
def main():
    ...
    out = Path(__file__).parent.parent / "results" / "students" / "uniform"
    ...
    if main_rank:
        print(f"uniform: {data.shape[0]} docs, vocab {vocab_size}, "
              f"world {world}, accum {accum}", flush=True)

    eval_files = sorted(glob.glob(str(root / "uniform_shard*_chunk009.npz")))
    ...
    if main_rank:
        print(f"uniform training done: final heldout loss {run_eval():.4f}",
              flush=True)
    if world > 1:
